...
let props = null, worldMap = null, heist = null, police = null;
if (WORLD) {
  ...
  police = createPolice(scene, model);
  heist = createHeist(scene, model, missions, camera, police);
  worldMap = createWorldMap(model, missions, heist, police);
  ...
  window.__world = { geom: worldGeom, traffic: worldTraffic, peds: pedestrians, model, missions, heist, police };
...
const GAME_FILES = [
  ...
  'src/worldmap.js', 'src/heist.js', 'src/police.js',
...
function loop(now) {
  ...
  // collide against traffic — and the police fleet (patrolling or chasing)
  const copCars = WORLD && police ? police.cars : null;
  ...
  if (WORLD) {
    ...
    if (police) police.update(dt, st ? st.pos : null, perfTime);
